#!/bin/sh
# Starts mcp-inspector in background, waits for qbit-mcp to bind its
...
# group so inspector + air + qbit-mcp all die together on docker stop.

set -e

if [ ! -d "/src/cmd/qbit-mcp" ]; then
  echo "entrypoint: /src/cmd/qbit-mcp missing — bind-mount the qbit-mcp repo at /src" >&2
  exit 1
fi
...
# Waits for qbit-mcp to bind the MCP port, then prints a copy-paste
...
(
  ...
  while ! nc -z 127.0.0.1 "${MCP_PORT}" 2>/dev/null; do
    i=$((i + 1))
    if [ "${i}" -gt 600 ]; then
      echo "devserver: qbit-mcp did not bind port ${MCP_PORT} within 60s; skipping inspector URL" >&2
      exit 0
    fi
    sleep 0.1
  done
  ...
  # qbit-mcp mounts the MCP handler there, not at root.
...
exec air -c /etc/qbit-mcp/air.toml
